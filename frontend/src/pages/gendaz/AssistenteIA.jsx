...
function gerarRespostaLocal(intencao, texto, contexto) {
  const { cliente, agendamentos, dashboard, servicos, profissionais, promos } = contexto
  ...
  switch (intencao) {
    ...
    case 'sobre': {
      return {
        resposta: `Sou a assistente de IA do estabelecimento, ${nome}! Posso ajudar com:\n\n• Agendar serviços\n• Reagendar compromissos\n• Cancelar agendamentos\n• Listar serviços e preços\n• Consultar promoções\n\nBasta me dizer o que precisa!`,
        sugestoes: ['Quero agendar', 'Ver serviços', 'Ver promoções'],
      }
    }
    case 'agradecimento': {
      return { resposta: `Por nada, ${nome}! 😊 Estou sempre aqui quando precisar. Bons momentos!` }
    }
    ...
    case 'meus_agendamentos': {
      ...
      const lista = agendamentos.map((a, i) =>
        `${i + 1}. ${a.servico || a.servicoNome || 'Serviço'} — ${a.data ? new Date(a.data + 'T12:00:00').toLocaleDateString('pt-BR') : '?'} às ${a.hora || '?'} com ${a.profissional || a.profissionalNome || '?'} [${a.status}]`
      ).join('\n')
      return {
    ...
    case 'cancelar': {
      ...
      return {
        ...
        sugestoes: ['Ir para Agenda'],
        acao: { tipo: 'CANCELAR', dados: { agendamentos } },
      }
    }
    case 'reagendar': {
      if (!agendamentos || agendamentos.length === 0) {
      ...
      return {
        ...
        sugestoes: ['Ir para Agenda'],
        acao: { tipo: 'REAGENDAR', dados: { agendamentos } },
      }
    }
    case 'promocoes': {
...
export default function AssistenteIA() {
  const { cliente, agendamentos, dashboard, enviarMensagemIA, carregarPreferenciasIA, servicos, profissionais, beneficios } = useContext(ClienteGendazContext)
  ...
  useEffect(() => {
    ...
    setMensagens([{
      id: 1,
      origem: 'ia',
      texto: `Olá, ${nome}! 👋 Sou sua assistente de IA. Posso ajudar com agendamentos, preços, serviços, horários e promoções. Como posso ajudá-lo?`,
    }])
  }, [cliente])

  useEffect(() => {
    carregarPreferenciasIA().then(setPreferencias).catch(() => {})
  ...
  const handleEnviar = useCallback(async (e) => {
    ...
    try {
      ...
      let resposta
      try {
        const historicoChat = [...mensagens, novaMensagem].map((m) => ({
          role: m.origem === 'ia' ? 'assistant' : 'user',
          content: m.texto,
        }))
        ...
        resposta = gerarRespostaLocal(intencao, textoUsuario, {
          cliente,
          agendamentos,
          dashboard,
          servicos: servicos || [],
        ...
        <article className="gendaz-panel">
          ...
          <div className="gendaz-stack">
            <div className="gendaz-mini-card">
              <strong>Profissional favorito</strong>
              <span>{preferencias?.profissionalFavorito?.nome || preferencias?.profissionalFavorito || 'Ainda não definido'}</span>
            </div>
            <div className="gendaz-mini-card">
              <strong>Serviço favorito</strong>
              <span>{preferencias?.servicoFavorito?.nome || preferencias?.servicoFavorito || 'Ainda não definido'}</span>
            </div>
            <div className="gendaz-mini-card">
              <strong>Dias preferidos</strong>
              <span>{Array.isArray(preferencias?.diasPreferidos) ? preferencias.diasPreferidos.join(', ') : preferencias?.diasPreferidos || 'Ainda não definido'}</span>
            </div>
            <div className="gendaz-mini-card">
              <strong>Horários preferidos</strong>
              <span>{preferencias?.horarioPreferido || 'Ainda não definido'}</span>
            </div>
            <div className="gendaz-mini-card">
              <strong>Frequência</strong>
              <span>{preferencias?.frequencia || 'Ainda não definido'}</span>
            </div>
          </div>
        </article>
      </div>
