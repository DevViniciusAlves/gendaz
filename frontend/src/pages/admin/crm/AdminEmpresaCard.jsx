...
function formatarPlano(plano, quantidade) {
  const nome = (plano || 'BASICO').toString().toUpperCase()
...
export default function AdminEmpresaCard({ cliente, onEnviarMensagem, onVerHistorico }) {
  const seg = SEGMENTO_COLORS[cliente.segment] || SEGMENTO_COLORS.regular
  const iniciais = (cliente.empresaNome || 'EM').substring(0, 2).toUpperCase()
  ...
  return (
    <article className="crm-card">
      <header className="crm-card-header">
        <div className="crm-card-avatar"><Building2 size={16} /></div>

        <div className="crm-card-headline">
          <div className="crm-card-name-row">
            <span className="crm-card-name">{cliente.empresaNome}</span>
          ...
          {cliente.nome && (
            <div className="crm-card-contact" style={{ marginTop: 2 }}>
              <span>Responsável: {cliente.nome}</span>
            </div>
          )}

          <div className="crm-card-contact">
            {cliente.telefone && (
              <span><Phone size={10} /> {exibirTelefone(cliente.telefone)}</span>
            )}
            {cliente.email && (
              <span><Mail size={10} /> {cliente.email}</span>
            )}
          </div>
        </div>
      </header>
      ...
      <section className="crm-card-metrics">
        ...
        <div className="crm-card-metric">
          ...
          <div className="crm-card-metric-label">
            ...
            Ticket médio: {formatCurrency(gastoMedio)}
          </div>
        </div>
        <div className="crm-card-metric">
        ...
        <div className="crm-card-metric">
          <div className="crm-card-metric-value">{formatarPlano(cliente.planoAtual, cliente.quantidadePlanos ?? 1)}</div>
          <div className="crm-card-metric-label">Plano atual</div>
        </div>
      </section>

      <section className="crm-card-agenda">
        <span><strong>Último agendamento:</strong> {formatDateShort(cliente.ultimoAgendamentoData)}</span>
        <span><strong>Entrou no site:</strong> {formatDateTime(cliente.ultimaEntradaSite)}</span>
      </section>
